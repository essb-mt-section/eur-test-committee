...
print_table <- function(n_questions, schemata) {
  ...
  while(TRUE) {
    ...
    df[1:20, paste0("risbo.", c)] = grades(scores, schemata$risbo)
    df[1:20, paste0("remindo.", c)] = grades(scores, schemata$remindo)
    x = x + 20
    c = c + 1
...
Grades are calculated from score using a single linear interpolation between the point of the passing grade, $P_1$, and the highest possible grade, $P_2$. The smallest score of passing the exam has to fulfil the criterion of at least 55% knowledge, that is, 55% correct questions after guessing correction. The calculated passing score will to be rounded up. 

For example, if we have $n=$ **40 questions with 4 choices**, the guessing correction is set to $c_g=40/4=10$. With a passing criterion $k_p=.55$ and a passing grade $g_{p}=5.5$, the passing score $s_p$ is:

$$s_p = k_p ( n - c_g) + c_g = .55\, (40 -10) + 10 = 26.5 \rightarrow 27$$ 

Thus, if the highest possible grade is $g_{h}=10$, the interpolation used the following points: $$P_1 = (s_p, g_p)=(27,5.5)$$ $$P_2 =(n, g_h)=(40,10) $$


## Remindo's method

Remindo's method\footnote{Please note that I have only access to the grades of a few Remindo exams and I don't know whether that applies to all Remindo exams.} is somewhat different from the method used by Risbo: 

1. If answering options of the some questions will excluded after the exam, the guessing correction will be adapted accordingly.
...
3. The method uses a bi-linear interpolation. That is, one interpolation for all score between the pass-fail criteria  (see above) and separate linear interpolation for the scores between the guessing rate and the pass-fail criteria. This method ensures if students that have one question more correct than the guessing rate receive grade higher than 1.


Find below the grading procesure of multiple choise exam often used at the DEPCS. An implementation of the grading in `R` to calculate the grades of any exam can be found [here](http://)
\newpage
...
```
## MC exam with `r n_questions` questions and `r n_choices` choises
`r risbo_formular(s)`
